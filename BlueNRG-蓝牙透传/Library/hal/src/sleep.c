...
static void BlueNRG_InternalSleep(SleepModes sleepMode, uint8_t gpioWakeBitMask, uint8_t sleepTimerEnabled)
{
  ...
  uint32_t RTC_TPR2_saved, RTC_TPR3_saved, RTC_TPR4_saved, RTC_TIN_saved, RTC_TWK_saved;
  ...
  RTC_TWK_saved = RTC->TWK;
  /* MFTX */
  T1CRA_saved = MFTX1->TNCRA;
  T1CRB_saved = MFTX1->TNCRB;
  T1PRSC_saved = MFTX1->TNPRSC;
  T1CKC_saved = MFTX1->TNCKC;
  T1MCTRL_saved = MFTX1->TNMCTRL;
  T1ICTRL_saved = MFTX1->TNICTRL;
  T2CRA_saved = MFTX2->TNCRA;
  T2CRB_saved = MFTX2->TNCRB;
  T2PRSC_saved = MFTX2->TNPRSC;
  T2CKC_saved = MFTX2->TNCKC;
  T2MCTRL_saved = MFTX2->TNMCTRL;
  T2ICTRL_saved = MFTX2->TNICTRL;
  /* WDT */
  WDG_LR_saved = WDG->LR;
  WDG_CR_saved = WDG->CR;
  WDG_LOCK_saved = WDG->LOCK;
  /* DMA */
  for (i=0; i<8; i++) {
  ...
  if (wakeupFromSleepFlag) {
    ...
    memcpy((void *)NVIC->IP, NVIC_IPR_saved, sizeof(NVIC_IPR_saved)); // ??? Change with the osal function
    *(volatile uint32_t *)SHPR3_REG = PENDSV_SYSTICK_IPR_saved;
    ...
    RTC->TWK = RTC_TWK_saved; 
    /* MFTX */
    MFTX1->TNCRA = T1CRA_saved;
    MFTX1->TNCRB = T1CRB_saved;
    MFTX1->TNPRSC = T1PRSC_saved;
    MFTX1->TNCKC = T1CKC_saved;
    MFTX1->TNMCTRL = T1MCTRL_saved;
    MFTX1->TNICTRL = T1ICTRL_saved;
    MFTX2->TNCRA = T2CRA_saved;
    MFTX2->TNCRB = T2CRB_saved;
    MFTX2->TNPRSC = T2PRSC_saved;
    MFTX2->TNCKC = T2CKC_saved;
    MFTX2->TNMCTRL = T2MCTRL_saved;
    MFTX2->TNICTRL = T2ICTRL_saved;
    /* WDT */
    WDG->LR = WDG_LR_saved;
    WDG->CR = WDG_CR_saved;
                      ...
                      uint32_t sleep_time)
{
  ...
  /*I/O唤醒源的设置  add by zhangsb, 2018-05-14 */
  if (gpioWakeBitMask) {
    SYSTEM_CTRL->WKP_IO_IS = gpioWakeLevelMask;//设置唤醒IO电平
    SYSTEM_CTRL->WKP_IO_IE = gpioWakeBitMask;//使能IO唤醒
  } else {
    SYSTEM_CTRL->WKP_IO_IS = 0;
    SYSTEM_CTRL->WKP_IO_IE = 3; // Only wakeup source are GPIO9-GPIO10 that are also Jlink pins
  }

  /* 定时器唤醒源add by zhangsb, 2018-05-14 */
  if (sleep_time) {
