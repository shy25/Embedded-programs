...
*         This parameter can be GPIO_Pin_x where x can be (0..14).
...
* @param  GPIO_Pin: specifies the pin bits to be written.
*         This parameter can be GPIO_Pin_x where x can be (0..14).
* @retval None
*/
...
void GPIO_ResetBits(uint32_t GPIO_Pin)
{
  ...
  CLEAR_BIT(GPIO->DATA, GPIO_Pin);
}

/**
* @brief  Sets or clears the selected IO pin bit.
* @param  GPIO_Pin: specifies the IO bit to be written.
*         This parameter can be GPIO_Pin_x where x can be (0..14).
* @param  BitVal: specifies the value to be written to the selected bit.
*         This parameter can be one of the #ref BitAction enum values
* @retval None
*/
...
* @param  GPIO_Pin: specifies the IO bit to be toggled.
*         This parameter can be GPIO_Pin_x where x can be (0..14).
* @retval None
*/
void GPIO_ToggleBits(uint32_t GPIO_Pin)
{  
  /* Check the parameters */  
  assert_param(IS_GPIO_PINS(GPIO_Pin));
...
{
  /* Reset GPIO init structure parameters values */
  GPIO_EXTIInitStruct->GPIO_Pin = 0x00;
...
* @retval None
*/
void GPIO_EXTICmd(uint32_t GPIO_Pin, FunctionalState NewState)
{
  /* Check the parameters */
  assert_param(IS_GPIO_PINS(GPIO_Pin));
  
  if (NewState != DISABLE) {
    SET_BIT(GPIO->IE, GPIO_Pin);
  }
  else {
    CLEAR_BIT(GPIO->IE, GPIO_Pin);
  }
}
...
FlagStatus GPIO_GetITPendingBit(uint32_t GPIO_Pin)
{
  ...
  enablestatus =  READ_BIT(GPIO->IE, GPIO_Pin);
  
  if ((READ_BIT(GPIO->MIS, GPIO_Pin) != (uint32_t)RESET) && (enablestatus != (uint32_t)RESET)) {
    return SET;
  }
  else {
    return RESET;
