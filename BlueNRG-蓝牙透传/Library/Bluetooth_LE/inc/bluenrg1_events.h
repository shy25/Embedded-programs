...
Range: 0x0000-0x0EFF (0x0F00 - 0x0FFF Reserved for future use)
  ...
  * @retval None
  * @note Please note: this callback is called from an interrupt service routine context, so minimal activities should be done to record the event and process it later to avoid disruption on the system timings
*/
...
mode has been completed. The Connection_Handle will be a Connection_
Handle for an ACL connection. The Encryption_Enabled event parameter
...
Host has indicated support for Secure Connections in the Secure_Connections_
Host_Support parameter. When Secure_Connections_Host_Support is
...
0x03-0xFF Reserved.
  ...
  * @note Please note: this callback is called from an interrupt service routine context, so minimal activities should be done to record the event and process it later to avoid disruption on the system timings
*/
void hci_encryption_change_event_isr(uint8_t Status,
                                     uint16_t Connection_Handle,
...
specified by the Connection_Handle event parameter. The Connection_
Handle shall be for an ACL connection.
...
the Version event parameter shall be LMP VersNr parameter, the Manufacturer_
Name event parameter shall be the CompId parameter, and the Subversion
...
event parameter shall be Link Layer VersNr parameter, the Manufacturer_
Name event parameter shall be the CompId parameter, and the Subversion
...
Range: 0x0000-0x0EFF (0x0F00 - 0x0FFF Reserved for future use)
  ...
  * @note Please note: this callback is called from an interrupt service routine context, so minimal activities should be done to record the event and process it later to avoid disruption on the system timings
*/
void hci_read_remote_version_information_complete_event_isr(uint8_t Status,
                                                            uint16_t Connection_Handle,
                                                            uint8_t Version,
                                                            uint16_t Manufacturer_Name,
                                                            uint16_t Subversion);
...
Range: 0x0000-0x0EFF (0x0F00 - 0x0FFF Reserved for future use)
  ...
  * @note Please note: this callback is called from an interrupt service routine context, so minimal activities should be done to record the event and process it later to avoid disruption on the system timings
*/
void hci_encryption_key_refresh_complete_event_isr(uint8_t Status,
                                                   uint16_t Connection_Handle);
/**
 * @}
 */
...
Time Range: 7.5 msec to 4000 msec.
  * @param Conn_Latency Connection latency for this connection.
Range: 0x0006 to 0x0C80
Time = N * 1.25 msec
Time Range: 7.5 msec to 4000 msec.
...
and send information from multiple devices in one LE Advertising Report event.
  ...
  * @note Please note: this callback is called from an interrupt service routine context, so minimal activities should be done to record the event and process it later to avoid disruption on the system timings
*/
void hci_le_advertising_report_event_isr(uint8_t Num_Reports,
...
0x0C81 - 0xFFFF Reserved for future use
  ...
  * @note Please note: this callback is called from an interrupt service routine context, so minimal activities should be done to record the event and process it later to avoid disruption on the system timings
*/
void hci_le_connection_update_complete_event_isr(uint8_t Status,
                                                 uint16_t Connection_Handle,
                                                 uint16_t Conn_Interval,
                                                 uint16_t Conn_Latency,
                                                 uint16_t Supervision_Timeout);
...
Range: 0x0000-0x0EFF (0x0F00 - 0x0FFF Reserved for future use)
  ...
  * @note Please note: this callback is called from an interrupt service routine context, so minimal activities should be done to record the event and process it later to avoid disruption on the system timings
*/
void hci_le_read_remote_used_features_complete_event_isr(uint8_t Status,
                                                         uint16_t Connection_Handle,
...
Range: 0x0000-0x0EFF (0x0F00 - 0x0FFF Reserved for future use)
  ...
  * @note Please note: this callback is called from an interrupt service routine context, so minimal activities should be done to record the event and process it later to avoid disruption on the system timings
*/
void hci_le_long_term_key_request_event_isr(uint16_t Connection_Handle,
 ...
 */
/**
 *@defgroup ACI_events ACI events
 *@brief ACI events layer.
 *@{
 */
/* ACI events */
...
typedef struct Attribute_Group_Handle_Pair_t_s {
  ...
  uint16_t Group_Handle /** Group handle */;
...
timeout has occurred so that the upper layer can decide to disconnect the link.
  ...
  * @note Please note: this callback is called from an interrupt service routine context, so minimal activities should be done to record the event and process it later to avoid disruption on the system timings
*/
void aci_gap_pairing_complete_event_isr(uint16_t Connection_Handle,
                                        uint8_t Status);
/**
...
allow the slave to continue the pairing process with the master.
  ...
  * @note Please note: this callback is called from an interrupt service routine context, so minimal activities should be done to record the event and process it later to avoid disruption on the system timings
*/
void aci_gap_bond_lost_event_isr(void);
/**
...
- For Name Discovery Procedure:
  ...
  * @note Please note: this callback is called from an interrupt service routine context, so minimal activities should be done to record the event and process it later to avoid disruption on the system timings
*/
void aci_gap_proc_complete_event_isr(uint8_t Procedure_Code,
                                     uint8_t Status,
                                     uint8_t Data_Length,
                                     uint8_t Data[]);
/**
...
/**
  * @brief Each time BlueNRG-MS FW stack raises the error code
BLE_STATUS_INSUFFICIENT_RESOURCES (0x64), the
Evt_Blue_Gatt_Tx_Pool_Available event is generated as soon as there are at least two
