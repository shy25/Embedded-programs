...
ErrorStatus SdkEvalSpiInit(uint32_t baudrate)
{
  ...
  CRMU_PeripheralClockCmd(CRMU_PERIPH_GPIO | CRMU_PERIPH_SPI, ENABLE);   
...
ErrorStatus SdkEvalSpiDmaInit(uint32_t baudrate)
{
  ...
  /* Enable SPI and GPIO clocks */
  CRMU_PeripheralClockCmd(CRMU_PERIPH_GPIO | CRMU_PERIPH_SPI | CRMU_PERIPH_DMA, ENABLE);   
  ...
  DMA_ITConfig(DMA_CH_SPI_RX, DMA_IT_TC, ENABLE);
...
void DmaSpiHandlerCallback(void)
{
  ...
  if(DMA_GetITStatus(DMA_CH_SPI_RX_IT_TC)) {

    /* Clear pending bit */
    DMA_ClearITPendingBit(DMA_CH_SPI_RX_IT_TC);
...
ErrorStatus SdkEvalSpiIrqInit(uint32_t baudrate)
{
  ...
  SPI_FifoInterruptLevelConfig(SPI_FIFO_LEV_1, SPI_FIFO_LEV_1);
