...
{   
  ...
  if(SDK_EVAL_I2C == I2C2) {
    CRMU_PeripheralClockCmd(CRMU_PERIPH_GPIO | CRMU_PERIPH_I2C2, ENABLE);
  }
  else {
    CRMU_PeripheralClockCmd(CRMU_PERIPH_GPIO | CRMU_PERIPH_I2C1, ENABLE);
  }
...
ErrorStatus SdkEvalI2CWrite(uint8_t* pBuffer, uint8_t DeviceAddr, uint8_t RegisterAddr, uint8_t NumByteToWrite)
{
  ...
  while (((I2C_Type*)SDK_EVAL_I2C)->CR & I2C_CR_FTX);
...
{   
  ...
  /* Enable I2C and GPIO clocks */
  if(SDK_EVAL_I2C == I2C2) {
    CRMU_PeripheralClockCmd(CRMU_PERIPH_GPIO | CRMU_PERIPH_I2C2 | CRMU_PERIPH_DMA, ENABLE);
  }
  else {
    CRMU_PeripheralClockCmd(CRMU_PERIPH_GPIO | CRMU_PERIPH_I2C1 | CRMU_PERIPH_DMA, ENABLE);
  }
  ...
  DMA_ITConfig((DMA_CH_Type*)DMA_CH_I2C_RX, DMA_IT_TC, ENABLE);
  DMA_ITConfig((DMA_CH_Type*)DMA_CH_I2C_TX, DMA_IT_TC, ENABLE);
...
void DmaI2CHandlerCallback(void)
{
  ...
  if(DMA_GetITStatus(DMA_CH_I2C_RX_IT_TC)) {

    /* Clear pending bit */
    DMA_ClearITPendingBit(DMA_CH_I2C_RX_IT_TC);
  ...
  if(DMA_GetITStatus(DMA_CH_I2C_TX_IT_TC)) {

    /* Clear pending bit */
    DMA_ClearITPendingBit(DMA_CH_I2C_TX_IT_TC);
